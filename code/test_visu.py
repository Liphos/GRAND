...
for file in tqdm(range(len(list_f))):
    ...
    G = tg.data.Data(x=torch.tensor(antenna_pos, dtype=torch.float32), edge_index=torch.tensor(edge_index, dtype=torch.long).t().contiguous(), y=1)
    """
    data_graphs = [G, G, G]
    ...
    #plt.plot(efield_loc[:, 0], efield_loc[:, 1:])
    #plt.show(block=True)
    """   
print(n_samples)